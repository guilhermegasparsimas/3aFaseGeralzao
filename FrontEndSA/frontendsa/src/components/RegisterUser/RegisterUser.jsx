...
const RegisterUser = () => {
  ...
  const isPassowrdValid = () => password.length >= 8 && password === confirmPassword
  ...
  return (
    <div className='2-full max-w-md p-6 bg-white rounded-xl shadow-lg'>
        ...
        <form onSubmit={handleSubmit} className='space-y-4'>
          {/* registrar email */}
            ...
            <div>
                ...
                className='w-full p-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500' />
                {!isPasswordMatch &&(
                  <p className='text-red-500 text-sm mt-1'>As senhas não correspondemm!</p>
                )}
            </div>
            <div>
              <button
                type='submit'
                disabled={isSaving}
                className={`w-full p-2 rounded-lg text-white ${isSaving ? 'bg-gray-400 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700'} transition-colors cursor-pinter`}
              >
